# Program for linking GSA with Compos
...
def gsa2compos2gsa():
    ...
    search_list = str(gsa_model.get_all_saved_lists()[0]).split("\t")[4].split(" ")
    member_list = []
    ...
    sections_dictionary = {}
    for i in range(1, 93):  # create sections dictionary: range needs to match model
    ...
    with open('C:\\Temp\\compos_file.csv', 'w') as compos_file:
        compos_file.write("! This file was originally written by Compos version 8.6.0.7")
        ...
        for i in range(len(member_list)):
            ...
            beam_lenght = str(member_list[i][2])
            compos_file.write("\nMEMBER_TITLE,MEMBER-" + beam_number +
                              ",Latitude **  Longitude **,Note")
            compos_file.write("\nDESIGN_OPTION,MEMBER-" + beam_number +
                              ",EN1994-1-1:2004,UNPROPPED,BEAM_WEIGHT_YES,SLAB_WEIGHT_YES,"
                              "SHEAR_DEFORM_NO,THIN_SECTION_YES,2.00000,2.00000")
            ...
            compos_file.write("\nCRITERIA_BEAM_SIZE_LIMIT,MEMBER-" + beam_number +
                              ",20.0000,100.000,10.0000,50.0000")
            compos_file.write("\nCRITERIA_OPTIMISE_OPTION,MEMBER-" + beam_number +
                              ",MINIMUM_WEIGHT")
            compos_file.write("\nCRITERIA_SECTION_TYPE,MEMBER-" + beam_number + ",255")
            compos_file.write("\nCRITERIA_FREQUENCY,MEMBER-" + beam_number +
                              ",CHECK_NATURAL_FREQUENCY,4.00000,1.00000,0.100000")
            compos_file.write("\nBEAM_STEEL_MATERIAL_STD,MEMBER-" + beam_number + ",S355")
            compos_file.write("\nBEAM_WELDING_MATERIAL,MEMBER-" + beam_number + ",Grade 35")
            compos_file.write("\nBEAM_SPAN_LENGTH,MEMBER-" + beam_number + ",1," + beam_lenght)
            compos_file.write("\nBEAM_SECTION_AT_X,MEMBER-" + beam_number + ",1,1,0.000000,"
                              + beam_section + ",TAPERED_NO")
            ...
            compos_file.write("\nFINAL_RESTRAINT_2ND_BEAM,MEMBER-" + beam_number +
                              ",SEC_BEAM_AS_REST")
            compos_file.write("\nFINAL_END_FLANGE_FREE_ROTATE,MEMBER-" + beam_number +
                              ",FREE_TO_ROTATE")
            compos_file.write("\nSLAB_CONCRETE_MATERIAL,MEMBER-" + beam_number +
                              ",C40/50,NORMAL,CODE_DENSITY,2.40000,NOT_APPLY,0.330000,"
                              "CODE_E_RATIO,CODE_STRAIN")
            compos_file.write("\nSLAB_DIMENSION,MEMBER-" + beam_number +
                              ",1,1,0.000000,0.130000,1.00000,1.00000,TAPERED_YES,EFFECTIVE_WIDTH_NO")
            ...
            compos_file.write("\nDECKING_CATALOGUE,MEMBER-" + beam_number +
                              ",RLD,Ribdeck AL (0.9),S280,90.0000,DECKING_JOINTED,JOINT_NOT_WELD")
            ...
            compos_file.write("\nLOAD,MEMBER-" + beam_number + ",Uniform,Line,1.00000,1.50000,"
                              + str(dead_load) + "," + str(live_load) +
                              ",Line,1.00000,1.50000,3.00000,5.00000,3.00000,4.50000,6.00000,5.00000")
            compos_file.write("\nFLOOR_RESPONSE,MEMBER-" + beam_number + ",FLOOR_RESPONSE_ANALYSIS_NO")
            compos_file.write("\nEC4_DESIGN_OPTION,MEMBER-" + beam_number +
                              ",SHRINKAGE_DEFORM_EC4_YES,IGNORE_SHRINKAGE_DEFORM_NO,"
                              "APPROXIMATE_E_RATIO_YES,United Kingdom,CLASS_N,1.10000,0.550000,"
                              "28.0000,1.00000,36500.0,36500.0,50.0000,50.0000")
    ...
    compos_model.Open('C:\\Temp\\compos_file.csv')
    compos_results = []
    for i in range(len(member_list)):
        ...
        compos_results.append([member_name[7:], section])   # member[7:] strips "MEMBER-"
    compos_model.SaveAs("C:\\Temp\\compos_results.csv")
    compos_model.Close()
    compos_model = None
    ...
    for i in range(len(compos_results)):
        ...
        new_section = sections_dictionary[member_new_section]    # get section number of Compos result
