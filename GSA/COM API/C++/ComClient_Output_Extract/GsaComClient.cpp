  ...
  try {
    ...
    for (int ele = 1; ele <= Highest; ele++) {
      ...
      if (elemExist) {
        ...
        HRESULT iStat =
            pObj->Output_Init(OP_INIT_1D_AUTO_PTS, "default", _bstr_t("A1"),
                              14002001, NumberOfIntermediatPoint);
        long Totpos = pObj->Output_NumElemPos(ele);
        for (int pos = 0; pos < Totpos; pos++) {
          CString val = pObj->Output_Extract(ele, pos);
          CString Location;
          Location.Format(_T("%d"), pos);
          WriteString(theFile,
                      _T("At location ") + Location + _T(": FX(kN)=") + val);
        }
        // FY
        iStat = pObj->Output_Init(OP_INIT_1D_AUTO_PTS, "default", _bstr_t("A1"),
                                  14002002, NumberOfIntermediatPoint);
        ...
        for (int pos = 0; pos < Totpos; pos++) {
          ...
          Location.Format(_T("%d"), pos);
          WriteString(theFile,
                      _T("At location ") + Location + _T(": FY(kN)=") + val);
        }
        // FZ
        iStat = pObj->Output_Init(OP_INIT_1D_AUTO_PTS, "default", _bstr_t("A1"),
                                  14002003, NumberOfIntermediatPoint);
        Totpos = pObj->Output_NumElemPos(ele);
        for (int pos = 0; pos < Totpos; pos++) {
          CString val = pObj->Output_Extract(ele, pos);
          CString Location;
          Location.Format(_T("%d"), pos);
          WriteString(theFile,
                      _T("At location ") + Location + _T(": FZ(kN)=") + val);
        }

        // MXX
        iStat = pObj->Output_Init(OP_INIT_1D_AUTO_PTS, "default", _bstr_t("A1"),
                                  14002005, NumberOfIntermediatPoint);
        Totpos = pObj->Output_NumElemPos(ele);
        for (int pos = 0; pos < Totpos; pos++) {
          CString val = pObj->Output_Extract(ele, pos);
          CString Location;
          Location.Format(_T("%d"), pos);
          WriteString(theFile,
                      _T("At location ") + Location + _T(": MXX(kN-m)=") + val);
        }
        // MYY
        iStat = pObj->Output_Init(OP_INIT_1D_AUTO_PTS, "default", _bstr_t("A1"),
                                  14002006, NumberOfIntermediatPoint);
        Totpos = pObj->Output_NumElemPos(ele);
        for (int pos = 0; pos < Totpos; pos++) {
          CString val = pObj->Output_Extract(ele, pos);
          CString Location;
          Location.Format(_T("%d"), pos);
          WriteString(theFile,
                      _T("At location ") + Location + _T(": MYY(kN-m)=") + val);
        }
        // MZZ
        iStat = pObj->Output_Init(OP_INIT_1D_AUTO_PTS, "default", _bstr_t("A1"),
                                  14002007, NumberOfIntermediatPoint);
        Totpos = pObj->Output_NumElemPos(ele);
        for (int pos = 0; pos < Totpos; pos++) {
          CString val = pObj->Output_Extract(ele, pos);
          CString Location;
          Location.Format(_T("%d"), pos);
          WriteString(theFile,
                      _T("At location ") + Location + _T(": MZZ(kN-m)=") + val);
        }
      }
    }
    theFile.Close();
